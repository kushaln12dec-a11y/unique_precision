...
/** Hard cap on employee logs loaded for dashboard aggregation (newest first). */
const DASHBOARD_LOG_TAKE = 5000;
...
router.get("/summary", async (req, res) => {
  try {
    ...
    const isPrivileged = reqRole === "ADMIN" || reqRole === "ACCOUNTANT";
    const activeView =
      isPrivileged
        ? requestedView
    ...
    // Prefer computing only the view that will be returned (frontend refetches on view change).
    const includeAdmin = activeView === "ADMIN" || (!["OPERATOR", "PROGRAMMER", "QC"].includes(activeView));
    const includeOperator = activeView === "OPERATOR";
    const includeProgrammer = activeView === "PROGRAMMER";
    const includeQc = activeView === "QC";
    ...
    // Jobs need history for YTD / 3-month trends / previous-window deltas, plus open QC queue rows.
    const historyStart = startOfDay(
      new Date(
        Math.min(
          dateRange.start.getTime(),
          previousRangeStart.getTime(),
    ...
    // Logs only power "today" operator metrics + activity feed — keep the Prisma window tight.
    const logRangeStart = startOfDay(
      new Date(Math.min(dateRange.start.getTime(), todayStart.getTime()))
    );
    const logRangeEnd = endOfDay(
      new Date(Math.max(dateRange.end.getTime(), todayEnd.getTime()))
    );
    ...
    const [users, jobs, logs] = await prisma.$transaction([
      ...
      prisma.employeeLog.findMany({
        where: {
          startedAt: { gte: logRangeStart, lte: logRangeEnd },
        },
        orderBy: { startedAt: "desc" },
        take: DASHBOARD_LOG_TAKE,
        select: {
          id: true,
          role: true,
          activityType: true,
    ...
    const needTrend = includeAdmin || includeQc;
    const monthlyBuckets = needTrend ? getMonthBuckets(3) : [];
    ...
    const needTopOperator = includeAdmin || (isPrivileged && includeOperator);
    const needTopProgrammer = includeAdmin || (isPrivileged && includeProgrammer);
    const needTopQc = includeAdmin || (isPrivileged && includeQc);

    const topOperator = needTopOperator
      ? aggregateByKey(
      ...
      : undefined;
    const topProgrammer = needTopProgrammer
      ? aggregateByKey(
    ...
    const programmerFocusName =
      isPrivileged && includeProgrammer
        ? programmerFilter || topProgrammer?.name || currentUser.name
        : currentUser.name;
    const programmerFocusVariants = buildNameVariants(programmerFocusName);
    const qcFocusName =
      isPrivileged && includeQc
        ? topQc?.name || currentUser.name
        : currentUser.name;
    const qcFocusVariants = buildNameVariants(qcFocusName);

    const focusedOperatorLogs = includeOperator
      ? operatorLogsToday.filter((log) =>
          fieldMatchesVariants(log.userName || log.userEmail || "", operatorFocusVariants)
        )
      : [];
    const currentOperatorJobs = includeOperator
      ? filteredJobs.filter((job) => fieldMatchesVariants(job.assignedTo, operatorFocusVariants))
      : [];
    const currentProgrammerJobs = includeProgrammer
      ? filteredJobs.filter((job) => fieldMatchesVariants(job.createdBy, programmerFocusVariants))
      : [];
    const currentQcJobs = includeQc
      ? filteredJobs.filter((job) => fieldMatchesVariants(job.updatedBy, qcFocusVariants))
      : [];
    ...
    if (includeOperator) {
      const activeCurrentLog = focusedOperatorLogs
      ...
      const operatorHoursToday = round(sumBy(focusedOperatorLogs, (log) => (log.durationSeconds || 0) / 3600), 2);
      ...
      const operatorSeries = Array.from({ length: 8 }).map((_, index) => {
        ...
        const bucketLogs = focusedOperatorLogs.filter((log) => log.startedAt >= bucketStart && log.startedAt <= bucketEnd);
      ...
      operator = {
        profile: {
          ...
          badge: isPrivileged ? "Admin View" : "Operator",
    ...
    let programmer: Record<string, unknown> | undefined;
    ...
    let qc: Record<string, unknown> | undefined;
    if (includeQc) {
    ...
    if (includeAdmin && admin) result.admin = admin;
    if (includeOperator && operator) result.operator = operator;
    if (includeProgrammer && programmer) result.programmer = programmer;
    if (includeQc && qc) result.qc = qc;
